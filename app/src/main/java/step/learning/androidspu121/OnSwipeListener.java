...
public class OnSwipeListener implements View.OnTouchListener{
    ...
    public void onSwipeLeft()   {}  // якщо зробити абстрактними. то кожен кдас
    public void onSwipeRight()  {}  // має реалізувати УСІ методи. Порожні тіла
    public void onSwipeTop()    {}  // дозволяють переозначити ОКРЕМІ з ним
    ...
    private final class SwipeGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        // База для свайпів: Fling - торкання(e1)--проведення--відпускання(e2)
        @Override
        public boolean onFling(@NonNull MotionEvent e1, @NonNull MotionEvent e2, float velocityX, float velocityY) {
            // обробники подій повертають true, якщо це "їх подія" і вона була оброблена,
            // інакше false, що свідчить про потребу передвати подію іншим обробникам
            boolean result = false;
            // задача - визначити відстань, швидкість та напрям жесту
            // та накласти обмеження на те. щоб вважати жест свайпом.
            float dx = e2.getX() - e1.getX(); // відстань по горизонтялі (зі знаком)
            float dy = e2.getY() - e1.getY();
            // Сучасні екрани з великою роздільною здатністю можуть навіть жест торкання
            // (дотику) врахувати як проведення на невелику відстань
            // Відповідно. слід ввести обмеження на мінімальну відстань (довжину)
            // жесту для вважання його свайпом. Також бажано ввести на мінімальну швидкість
            // руху для того щоб відрізняти цей жест від scroll
            if (Math.abs(dx) > Math.abs(dy)) {
                // відстань (без знаку) по Х більша ніж по Y -- вважаємо жест горизотальним
                // а значіть далі перевіряємо обмеження на горизонтальну швидкість
...
onTouch - торканя, onFling - проведення, onLongTouch - довгй натиск
У той же час, достьб поширені події swipe (швидке проведення) не є базовим
